 ...
 * @author Steve
 */
public class ThreeKindLogic extends AbstractGameModeLogic {
   ...
   */
  public ThreeKindLogic(Settings settings) {
    super(settings);
  }

  /**
   * Ανοιγει τυχαια μια καρτα και αν ολα τα ζευγαρια τις ειναι στη μνημη του
   * παικτη
   ...
   */
  @Override
  public synchronized void compareCards(int k, ArrayList<JLabel> ol) {
    ...
    if (!cos) {
      ...
      if (x.equals(y) && x.equals(z) && openSeq == x.getOrder()) {
        equallity(k, l1, l2, l3, x, y, z);
      } else {
        unequallity();
      }
 ...
 αυξανει το σκορ του παικτη και τις αφαιρει απ'το HashMap map
 αλλα και απ'τα HashMap των αντιπαλων παικτων.
  ...
  private void equallity(int k, JLabel l1, JLabel l2, JLabel l3, Card x, Card y, Card z) {
    System.out.println("Congratulations " + pa[k].getName() + " found a three match!");
    SwingUtilities.invokeLater(new Runnable() {
      @Override
      public void run() {
        l1.setEnabled(false);
