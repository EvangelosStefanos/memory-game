 ...
 * @author Steve
 */
public class LogicQuartet extends Logic {
   ...
   */
  public LogicQuartet(Settings settings) {
    super(settings);
  }
  ...
  public synchronized void compareCards(int k, ArrayList<JLabel> ol) {
    ...
    if (!cos) {
      ...
      if (x.equals(y) && x.equals(z) && x.equals(m) && openSeq == x.getSequence()) {
        equallity(k, l1, l2, l3, l4, x, y, z, m);
      } else {
        unequallity();
      }
 ...
 αλλα και απ'τα HashMap των αντιπαλων παικτων.
  ...
  private void equallity(int k, JLabel l1, JLabel l2, JLabel l3, JLabel l4, Card x, Card y, Card z, Card m) {
    System.out.println("Congratulations " + pa[k].getName() + " found a quartet!");
    SwingUtilities.invokeLater(new Runnable() {
      @Override
      public void run() {
        l1.setEnabled(false);
